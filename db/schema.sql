-- schema.sql — структура хранилища для RAG (справочный файл для чтения).
--
-- На практике таблицу создаёт src/db.py::init_schema() динамически, потому
-- что размерность vector(N) зависит от бэкенда эмбеддингов (384 локально,
-- 1024 у Cohere, см. config.py) — статичный SQL-файл не может это выразить
-- без дублирования. Но структуру проще понять глазами именно тут.
--
